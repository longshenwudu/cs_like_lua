...
namespace com.calitha.goldparser
{
	...
	public class LALRParser
	{
        ...
        public string Re_i(string input)
        {
            ...
            if (System.Text.RegularExpressions.Regex.IsMatch(input, pattern))
            {
                ...
                var re = regex.Matches(input);
                foreach (System.Text.RegularExpressions.Match p in re)
                {
                    var pr = System.Text.Encoding.UTF8.GetBytes(p.Value);
                    var new_w = "";

                    {
                        char[] charbuffers = p.Value.ToCharArray();
                        byte[] buffer;
                        StringBuilder sb = new StringBuilder();
                        for (int i = 0; i < charbuffers.Length; i++)
                        {
                            buffer = System.Text.Encoding.Unicode.GetBytes(charbuffers[i].ToString());
                            sb.Append(String.Format("Éñ{0:X2}{1:X2}Áé", buffer[1], buffer[0]));
                        }
                        input = input.Replace(p.Value, "_" + sb.ToString() + "_");
                    }
                    //"_"+new_w);
                }
            }
